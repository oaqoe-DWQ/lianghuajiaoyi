...
from data_manager import update_system_status, save_trade_record, save_ai_analysis_record
...
def get_symbol_base():
    """从 TRADE_CONFIG['symbol'] 解析基础币种（如 'ETH/USDT:USDT' → 'ETH'）。"""
    try:
        sym = TRADE_CONFIG.get('symbol', 'BTC/USDT:USDT')
        if '/' in sym:
            return sym.split('/')[0]
    except Exception:
        pass
...
def get_timeframe_minutes():
    """解析 TRADE_CONFIG['timeframe'] 的分钟数，默认15。"""
    try:
...
def ensure_data_points_for_24h():
    """按当前周期设置 data_points 覆盖约24小时。"""
    minutes = get_timeframe_minutes()
    per_day = max(1, (24 * 60) // minutes)
    TRADE_CONFIG['data_points'] = per_day
...
def get_sentiment_indicators():
    ...
    try:
        ...
        # 将本策略周期映射到情绪接口粒度（若不支持则回退15m）
        tf_minutes = get_timeframe_minutes()
        if tf_minutes in [1, 3, 5, 15]:
            time_type = f"{tf_minutes}m"
        else:
            time_type = "15m"
...
def analyze_with_deepseek(price_data):
    ...
    - 本K线成交量: {price_data['volume']:.2f} {get_symbol_base()}
...
def wait_for_next_period():
    """等待到下一个K线整点，周期取自 TRADE_CONFIG['timeframe']（如 1m/3m/5m/15m）"""
    now = datetime.now()
    current_minute = now.minute
    current_second = now.second

    # 解析周期分钟数
    period_minutes = get_timeframe_minutes()
...
def trading_bot():
    ...
    print(f"{get_symbol_base()}当前价格: ${price_data['price']:,.2f}")
...
def main():
    ...
    # 按周期自动设置 data_points 覆盖约24小时
    ensure_data_points_for_24h()
    ...
    period_minutes = get_timeframe_minutes()
    print(f"执行频率: 每{period_minutes}分钟整点执行")
